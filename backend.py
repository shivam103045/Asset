...
import bcrypt
import requests
from flask import Flask, request, jsonify, Response, g, render_template, session, redirect, url_for
...
def make_token(uid):
    payload = {"sub": str(uid), "exp": datetime.now(timezone.utc) + timedelta(hours=JWT_HOURS)}
    ...
    for host in ["query1", "query2"]:
        try:
            ...
            if not ts:
                continue
            return [
                {"time": datetime.fromtimestamp(t, tz=timezone.utc).strftime("%H:%M"), "close": round(float(c), 2)}
...
<title>AssetEdge — Access</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{min-height:100vh;display:flex;align-items:center;justify-content:center;
     background:#08080f;font-family:Inter,sans-serif;color:#f0e8d0}
.card{width:100%;max-width:380px;padding:40px;background:#0f0f1a;
      border:1px solid rgba(201,168,76,.18);border-radius:12px}
h2{font-size:22px;font-weight:700;margin-bottom:6px}
h2 span{color:#c9a84c}
p{color:#6b6358;font-size:13px;margin-bottom:24px}
input{width:100%;padding:11px 14px;background:#161622;border:1px solid rgba(201,168,76,.2);
      border-radius:8px;color:#f0e8d0;font-size:14px;outline:none;margin-bottom:14px}
input:focus{border-color:#c9a84c}
button{width:100%;padding:11px;background:#c9a84c;color:#08080f;font-weight:700;
       font-size:14px;border:none;border-radius:8px;cursor:pointer}
.err{color:#c4534a;font-size:12px;margin-top:10px}
</style></head>
<body><div class="card">
<h2>Asset<span>Edge</span></h2>
<p>Private access only — enter your access key</p>
...
@app.before_request
def check_access():
    # Always allow the unlock route itself
    if request.path in ("/unlock",):
        return
    # Skip if no ACCESS_KEY is set (development mode)
    if not ACCESS_KEY:
        return
    # Allow if session is authenticated
    if session.get("access_granted"):
        return
    # Block everything — return the gate page
    if request.path.startswith("/api/"):
        return jsonify({"detail": "Unauthorized"}), 401
    return ACCESS_GATE_HTML.replace("%%ERROR%%", ""), 403
...
def unlock():
    if request.method == "POST":
        if request.form.get("key") == ACCESS_KEY:
            ...
            return redirect("/")
        return ACCESS_GATE_HTML.replace("%%ERROR%%", '<p class="err">Wrong access key — try again</p>'), 403
    return redirect("/")
...
def ping():
    return jsonify({"status": "ok", "service": "AssetEdge", "time": datetime.now(timezone.utc).isoformat()})
